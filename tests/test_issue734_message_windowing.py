...
def test_scroll_up_expands_local_window_before_server_pagination_and_preserves_scroll():
    ...
    # The #messages scroll handler reveals earlier content as the reader scrolls
    # up: expand the in-memory window first, then page older messages.
    assert "if(_messageHiddenBeforeCount()>0) _showEarlierRenderedMessages();" in UI_JS
    assert "else if(typeof _messagesTruncated!=='undefined' && _messagesTruncated && typeof _loadOlderMessages==='function') _loadOlderMessages();" in UI_JS
...
def test_window_state_participates_in_render_cache():
